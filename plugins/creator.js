...
if (command == 'creator') {
  ...
  const sentMsg = await conn.sendContactArray(m.chat, [
    [`${nomorown}`, `${await conn.getName(nomorown+'@s.whatsapp.net')}`, `👑 Developer Bot `, `🚫 Don't call me 🥺`, `irsyad@gmail.com`, `🇮🇩 Indonesia`, `🚀 -/`, `👤 Gada pawang nih senggol dong 😔`],
    [`${conn.user.jid.split('@')[0]}`, `${await conn.getName(conn.user.jid)}`, `🔥 Bot WhatsApp 🐣`, `📵 Don't spam/call me 😢`, `Nothing`, `🇮🇩 Indonesia`, `🚀 https://instagram.com/baukpandan_1324`, `🤖 Hanya bot biasa yang kadang suka eror ☺`]
  ], fakes)
